...
// Update resources generated by a digger working in a room
...
// Advance resources, called outside interrupt
void res_update(void)
{
	char	production = 4;

	// Natural oxygen in the upper layers
	...
	// Basic energy production to avoid locking player in a dead end
	if (!(res_update_cnt & 15))
		res_stored[RES_ENERGY] += 1;

	char heal = 0;

	// Main resource update loop for all diggers
	for(char i=0; i<32; i++)
	{
	...
	// Basic metal and healing resources to avoid locking player in a dead end
	res_update_cnt++;
	...
	// Main time game progress counter
	if (!(res_update_cnt & 1))
	{
		// time counter
		time_count++;
	...
	// Some random diggers are thirsty
	for(char i=0; i<2; i++)
	{
	...
	// Clip resources to storage space
	for(char i=0; i<NUM_RESOURCES; i++)
	{
